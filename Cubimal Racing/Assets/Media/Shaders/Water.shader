...
Shader "FX/Waterfall"
{
	Properties
	{
		...
		_SideAlphaTex ("Side Water Alpha Texture", 2D) = "white" {}
	...
	SubShader
	{
		...
		sampler2D _SideNoiseTex, _SideAlphaTex, _TopNoiseTex;
		float4 _SideAlphaTex_ST;
		...
		void vert(inout appdata_full v, out Input o)
        {
			...
			o.texcoord = TRANSFORM_TEX(v.texcoord, _SideAlphaTex);
        }
		...
		void surf(Input IN, inout SurfaceOutputStandard o)
		{
			...
			// clamped alpha
			fixed4 sideAlpha = tex2D(_SideAlphaTex, IN.texcoord);
			o.Alpha = clamp((color.a + combinedFoam + foamLine.a) * sideAlpha.a, 0, 1);
		}
		ENDCG
	}
	FallBack "Diffuse"
